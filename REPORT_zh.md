...
当前代码进一步支持 `--snapkv_head_aggregation mean|max|per_head`。`mean` 是原始简化实现，会把所有 head 平均成一个 `[seq_len]` 分数；`max` 则先得到每个 head 的 token 分数，再对 head 维取最大值，用来测试“某个 head 独有的重要 token 是否被均值稀释”的假设，但仍然让所有 head 共享同一组索引；`per_head` 是最接近完整 SnapKV 的路径，会保留 `[heads, seq_len]` 分数并让每个 attention head 独立 top-k、独立 gather K/V。得到 token 分数后，可以通过 `--snapkv_pooling_kernel` 对序列维度做长度保持的一维平均池化；默认值 `1` 表示不做 pooling，`3` 或 `5` 可用于测试局部连续性假设。随后只在中间区域做 top-k，最后将 sink、SnapKV middle、recent 合并去重并按原始位置排序，以保证压缩后的 K/V 仍保持原始 token 顺序。
